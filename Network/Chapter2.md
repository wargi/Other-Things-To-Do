...
- 따로따로 처리하기 위해 스위치는 두가지의 동작을 수행하는데
  ...
  2. 버퍼링
     ...
     - 버퍼를 사용한 처리를 실행하는 걸 말하는데, 쉽게 말해 충돌할 것 같은 프레임을 버퍼에 미리 저장 해주고 먼저 도착한 프레임을 송신하고 그 다음에 처리한다.

     - 하지만, 수신처가 같은 프레임이 계속적으로 올 경우 버퍼에 용량에 제한이 생기기 때문에 문제가 될 수 있다. 이런 경우에는 백 프레셔(Back Pressure) 또는 IEEE802.3x라는 규격을 사용하여 송신을 중지한다.

       > - 백 프레셔와 IEEE802.3x 두가지 방식으로 나뉘는 이유는 전이중 이더넷에 대응여부에 따라 나뉘게 되는데 대응한다면 IEEE802.3x 아니라면 백 프레셔(Back Pressure) 방식을 사용한다.

     (TIP) 버퍼: 일시적으로 데이터를 기록해 둘 수 있는 기억기기(메모리)

- 스위치를 공부해보면 '충돌 도메인'의 범위가 줄어 들겠다는 생각을 하였는데 책에서도 이 부분에 대해 설명해주는데 스위치를 사용하면 충돌이 발생 하지 않게 된다. 즉 충돌 도메인은 스위치에 의해 분할되어 데이터 통신의 효율을 높여준다.

## 8. 전이중 이더넷
- 스위치를 사용해 전이중 통신을 하는 것을 '전이중 이더넷'이라고 한다.

  > - 전이중 통신: 송신과 수신을 동시에 할 수 있는 방식
  > - 반이중 통신: 누군가가 송신 중일 때는 데이터를 송신하지 않으며 자신이 송신 중일 때는 충돌을 막기 위해 다른 컴퓨터가 송신해 오지 않는다. (CSMA/CD는 반이중 통신)

- 또, 여기서 생각해 볼 필요가 있는건 CSMA/CD인데 CSMA/CD는 충돌 가능성이 있지만 그에 비해, 스위치는 충돌이 발생하지 않기에 CSMA/CD를 사용할 필요가 없다. 
