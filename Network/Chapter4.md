...
> - ICMP는 에러 보고 프로토콜이라고도 불리며 네트워크의 제어와 관리등을 수행하는 프로토콜이다.
> - IP 데이터그램에 ICMP 메시지를 넣어 데이터를 주고받는다.
> - ICMP 메시지란
>
> > - ICMP에 사용되는 정보인데 이 정보를 IP 데이터그램의 페이로드에 넣는다.
> > - 보통 IP 데이터그램의 페이로드에는 'TCP 세그먼트'나 'UDP 데이터그램'이 들어가는데 이것들 대신에 ICMP 메시지를 넣어 보낸다.(최종적으로는 IP헤더 + ICMP 메시지)
> > - ICMP 메세지는 타입과 타입에 따른 코드로 되어 있는데.
> >   1. 타입: ICMP의 종류
> >   2. 코드: 타입에 따른 상세 내용

- ICMP의 종류

> - ICMP에는 크게 두 종류의 메시지가 있다.
>   1. Query 메시지
>   2. Error 메시지
>
> | 데이터 |          데이터           |            데이터             | 데이터 |
...
>
> > - 1, 2, 7번은 본래 정의되어 있지 않은 번호이다.
> > - 위의 것중에 0, 3, 5, 8, 11번이 자주 이용되는 것들이다.

- TTL

> - IP 헤더에 포함된 것들 중에 ICMP와 상당히 관련히 있는 항목이 TTL(Time To Live)이다.
>
> - IP 데이터그램의 TTL은 라우터를 경유할 때마다 1만큼씩 줄어들고, 0이되면 그 데이터그램은 파기된다.(Swift의 Reference Counting이 생각나네요,,ㅎ)
>
> - 카운팅이 필요한 이유는 라우팅에서 경로 정보에 오류가 일어날 수 있기 때문이다
>
>   > 예를 들어, 정적 라우팅 테이블에서 수동으로 경로를 입력할 때 중계 라우터를 들리는 경우가 있는데, 원래는 그렇게 되면, 수신처에 도달하지 않고 어딘가로 가버리든지 또는 같은 장소를 빙글빙글 계속해서 돌게 되어버리기 때문에 데이터그램은 영원히 네트워크 내에서 계속해서 끝나지 않고 무한하게 순환되어 버리게 된다.
>
